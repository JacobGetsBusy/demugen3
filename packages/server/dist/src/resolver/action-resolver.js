import { IntentType, GamePhase, TurnPhase, STARTING_LIFE, } from '@mugen/shared';
import { TurnEngine, GameEngine, } from '@mugen/shared';
export function createInitialGameState(lobby) {
    const players = lobby.players.map((lp) => ({
        id: lp.id,
        name: lp.name,
        life: STARTING_LIFE,
...
export function resolveIntent(state, playerId, intent) {
...
function resolvePreGameIntent(state, _playerId, intent) {
    ...
    switch (intent.type) {
        ...
        case IntentType.LOCK_TEAM:
            // Simplified — full implementation with PreGameManager integration
            return { ok: true, value: state };
        default:
            return { ok: false, error: `Invalid intent for pre-game: ${intent.type}` };
    }
}
function resolveGameIntent(state, playerId, intent) {
    switch (intent.type) {
