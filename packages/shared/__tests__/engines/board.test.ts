...
describe('BoardEngine', () => {
  ...
  describe('createBoardState', () => {
    it('default size — returns 30x30 empty grid', () => {
  ...
  describe('placeUnit', () => {
    ...
    it('valid placement at (29,29) — boundary of 30x30 grid', () => {
      const board = createBoardState();
      const pos: Position = { x: 29, y: 29 };
      const result = placeUnit(board, 'unit-1', pos);
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value.cells[29]![29]!.occupantId).toBe('unit-1');
      }
    });
  ...
  describe('moveUnit', () => {
    ...
    it('target out of bounds — returns error', () => {
      let board = createBoardState();
      const from: Position = { x: 29, y: 29 };
      const to: Position = { x: 30, y: 30 };
      const placed = placeUnit(board, 'unit-1', from);
      if (placed.ok) board = placed.value;
    ...
    it('move to boundary edge (28,29) → (29,29) — valid', () => {
      let board = createBoardState();
      const from: Position = { x: 28, y: 29 };
      const to: Position = { x: 29, y: 29 };
      const placed = placeUnit(board, 'unit-1', from);
      if (placed.ok) board = placed.value;
