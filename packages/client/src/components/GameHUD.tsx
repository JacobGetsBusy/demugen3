...
import type { UnitInstance } from '@mugen/shared';
...
function UnitCard({ unit }: { unit: UnitInstance }) {
  ...
  return (
    <button
      onClick={() => selectUnit(isSelected ? null : unit.card.id)}
      className={`flex flex-col items-start p-3 rounded-lg border transition text-left w-full ${
        isSelected
          ? 'border-mugen-accent bg-mugen-accent/10'
          : 'border-white/10 bg-mugen-bg hover:border-white/20'
      } ${unit.currentHp <= 0 ? 'opacity-30' : ''}`}
    >
      <span className="font-semibold text-sm truncate w-full">{unit.card.name}</span>
      <div className="flex items-center gap-3 mt-1 text-xs text-gray-400">
        <span className="flex items-center gap-1 text-mugen-danger">
          <Heart size={10} /> {unit.currentHp}/{unit.card.maxHp}
        </span>
        <span className="flex items-center gap-1 text-mugen-gold">
          <Swords size={10} /> {unit.card.atk}
        </span>
        <span className="flex items-center gap-1 text-mugen-mana">
          <Shield size={10} /> {unit.card.movement}
        </span>
      </div>
    </button>
  );
  ...
  return (
    <div className="flex gap-2 overflow-x-auto pb-2">
      {myPlayer.hand.cards.map((card) => (
        <div
          key={card.id}
          className="flex-shrink-0 w-28 bg-mugen-bg border border-white/10 rounded-lg p-2"
        >
...
export function GameHUD() {
  ...
  return (
    <div className="fixed inset-0 pointer-events-none">
      ...
      {/* Hover stats panel (left side) */}
      <UnitStatsPanel />

      {/* Left panel: my units */}
      <div className="absolute right-0 top-20 bottom-32 w-52 p-4 pointer-events-auto overflow-y-auto">
        <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Units</h3>
        <div className="space-y-2">
          {myPlayer?.units.filter((u) => u.currentHp > 0).map((u) => (
            <UnitCard key={u.card.id} unit={u} />
          ))}
        </div>
      </div>

      {/* Bottom: hand + life + phase controls */}
      <div className="absolute bottom-0 left-0 right-0 p-4 pointer-events-auto">
        <div className="bg-mugen-surface/90 backdrop-blur-sm rounded-xl border border-white/5 p-4">
