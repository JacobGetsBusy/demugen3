...
import type { GameState, UnitInstance, UnitCard } from '@mugen/shared';
import { CardType, AbilityType } from '@mugen/shared';

// Helper function to create a mock UnitCard
const createMockUnitCard = (id: string, name: string, cost: number, hp: number, atk: number): UnitCard => ({
  id,
  name,
...
const createMockGameState = (playerId: string, units: UnitInstance[], benchUnits: UnitCard[]): GameState => ({
  id: 'test-game',
...
describe('Client Store - Placement Integration', () => {
  ...
  it('should handle gameState updates for different players correctly', () => {
    ...
    const player1Units: UnitInstance[] = [
      {
        card: createMockUnitCard('unit-1', 'Knight', 5, 10, 3),
        currentHp: 10,
        position: { x: 5, y: 10 },
    ...
    const player2Units: UnitInstance[] = [
      {
        card: createMockUnitCard('unit-2', 'Archer', 4, 8, 4),
        currentHp: 8,
        position: { x: 25, y: 10 },
    ...
    const gameStateWithTwoPlayers: GameState = {
      id: 'test-game',
      phase: 'IN_PROGRESS' as any,
      turnPhase: 'MOVE' as any,
      currentPlayerIndex: 0,
    ...
    // Assert: Should show player 1's units (current player)
    ...
    // Assert: Should now show player 2's units
    expect(result.current.activeUnits).toHaveLength(1);
    expect(result.current.activeUnits[0]!.card.name).toBe('Archer');
    expect(result.current.activeUnits[0]!.position).toEqual({ x: 25, y: 10 });
  });
